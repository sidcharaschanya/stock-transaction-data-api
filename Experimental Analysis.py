...
import datetime
import random
import TransactionDataGenerator
...
def logTransactionsTest1(num):
    timeTaken = timeit.repeat(logTransactions(TransactionDataGenerator.TransactionDataGenerator.generateTransactionData(1)), repeat=num, number=1000)
    return timeTaken
...
def sortedTransactionsTest(stockName):
    timeTaken = timeit.timeit(sortedTransactions(stockName), number=1000)
    return timeTaken
...
def minTransactionsTest(stockName):
    timeTaken = timeit.timeit(minTransactions(stockName), number=1000)
    return timeTaken
...
def maxTransactionsTest(stockName):
    timeTaken = timeit.timeit(maxTransactions(stockName), number=1000)
    return timeTaken
...
def floorTransactionsTest1(stockName):
    ...
    timeTaken = timeit.timeit(floorTransactions(stockName, num), number=1000)
    return timeTaken
...
def ceilingTransactionsTest1(stockName):
    num = getMin(stockName)
    timeTaken = timeit.timeit(ceilingTransactions(stockName, num), number=1000)
    return timeTaken
...
def floorTransactionsTests(stockName):
    ...
    transactions = sortedTransactions(stockName)
...
def rangeTransactionsTest1(stockName):
    ...
    timeTaken = timeit.timeit(rangeTransactions(stockName, fromValue, toValue), number=1000)
    return timeTaken
...
def rangeTransactionsTest2(stockName, transactions):
    ...
    for i in range(1, numOfTransactions - 2):
        ...
        times.append(timeit.timeit(rangeTransactions(stockName, fromValue, toValue), number=1000))
